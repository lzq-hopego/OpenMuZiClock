...
String setTimeName;//存储闹钟界面的名字

int playstate=false;//当前不播放歌曲
//播放歌曲的任务句柄(闹钟页面使用)
...
//变更cpu占用页面的文字
void drawCpuPageText(String text1,String text2,String text3,String ip,String text4){
  ...
  clk.loadFont(simyou24);
  clk.fillRoundRect(30,105,60,32,7,0xfbbb);
  clk.drawString("温度:",60,123);
  clk.drawString(text4,160,123);
...
void drawPlaySongs(String setname){
   //清空屏幕
  reflashTFT();
  // 绘制标题
  clk.createSprite(240, 60);
...
void drawTimeAndOption(){
  ...
  clk.setTextColor(0xfcf8);
  clk.loadFont(simyou24);
  clk.drawString("设置",80,20);

  clk.setTextColor(TFT_RED);
  clk.drawString(setTimeName,140,20);
  // clk.drawString(String(text1),120,90);
  ...
  clk.setTextDatum(CC_DATUM);
// 时
  clk.drawNumber(hourl,20,80);
  clk.drawNumber(hourr,55,80);

  clk.drawString(":",80,74);
  clk.setTextColor(0xFC60);
// 分
  clk.drawNumber(minutel,110,80);
  clk.drawNumber(minuter,145,80);
//第几首歌
  clk.loadFont(clock_num_50);
  clk.setTextColor(TFT_RED);
  clk.drawNumber(0,180,88);
  clk.drawNumber(songNum,212,88);
//是否响铃

  if(RingingBell){
    clk.loadFont(simyou24);
    clk.drawString("*^____^*",180,200);
  }
...
void drawDateWeek(){
  ...
  if(RingingBell){
    ...
    if(hourl==hourH && hourr==hourL){
      if (minutel==minuteH && minuter==minuteL){
        if(secondH==0 && secondL==0){
          if(playstate==false){
            // Serial.println("响铃3");
            createPlaySongsTask();
            playstate=true;
...
//设置字体
  ...
  clk.drawString(title,120,30);
  // clk.drawString("李木子的时钟",120,30);
  clk.unloadFont();
